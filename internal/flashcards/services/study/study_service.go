...
type IStudyService interface {
	...
	MakeMatching(uid models.UserId, down models.CoolDown, size int) (models.Matching, error)

	// MakeSentence returns a text generated by a ***AI_SERVICE_NAME*** from the entire set of user cards with expired CoolDown.
	MakeSentence(generator IPalmService, uid models.UserId, down models.CoolDown, size int) (models.Sentence, error)
}
...
func (s *Service) GetNextRandomN(uid models.UserId, down models.CoolDown, n int) []models.FlashcardId {
...
func (s *Service) GetNextRandom(uid models.UserId, down models.CoolDown) (models.FlashcardId, error) {
	cards := s.GetNextRandomN(uid, down, 1)
	cardAmt := len(cards)
	if cardAmt == 0 {
		return 0, ErrNoCards
	} else if cardAmt > 1 {
...
func (s *Service) GetNextRandomDeck(uid models.UserId, id models.DeckId, down models.CoolDown) (models.FlashcardId, error) {
	cards, error := s.GetNextRandomDeckN(uid, id, down, 1)
...
func (s Service) MakeMatching(uid models.UserId, down models.CoolDown, size int) (models.Matching, error) {
	cardIds := s.GetNextRandomN(uid, down, size)
	cards := make(map[string]models.Flashcard)
	var words []string = nil
	var answers []string = nil

	for _, v := range cardIds {
		card, err := s.dsrv.GetFlashcardById(v)
		if err != nil {
			continue
		}
		_, ok := cards[card.W]
...
func (s Service) MakeSentence(generator IPalmService, uid models.UserId, down models.CoolDown, size int) (models.Sentence, error) {
	cardIds := s.GetNextRandomN(uid, down, size)
	cards := make(map[string]models.Flashcard)
	var words []string = nil
	...
	if len(cards) < 2 {
		return models.Sentence{}, ErrCardsAmt
	}
